...
## Vježba 5: Inventory Management

Napravi sustav za upravljanje zalihama:
- `Proizvod` s min/max količinama
- Automatsko naručivanje kad padne ispod minimuma
- Povijest kretanja robe
- ABC analiza (najvažniji proizvodi)
...
## Online učenje
- Real Python - realističan pristup s primjerima
- Python.org tutorials - službeni tutorijali
- W3Schools Python - brzi pregled sintakse
- GeeksforGeeks Python - detaljni članci

## Knjige (preporučene)
- "Python Crash Course" - Eric Matthes (početnici)
- "Fluent Python" - Luciano Ramalho (napredni)
- "Effective Python" - Brett Slatkin (best practices)
- "Python Cookbook" - David Beazley (recepti)

## Video resursi
- Corey Schafer (YouTube) - odličan za OOP
- Tech With Tim - Python projekti
- Programming with Mosh - Python tutorial

## Prakse i testiranje
- LeetCode - algoritamski zadaci
- HackerRank - Python challenges
- Codewars - Python kata
- Project Euler - matematički problemi

---

# Savjeti za učenje

## 1. Vježbaj svakodnevno
```python
# Napiši kod svaki dan, makar 30 minuta
# Bolje malo često nego puno rijetko
```

## 2. Napravi vlastite projekte
```python
# Ne samo tutorial, već stvarne aplikacije:
# - Todo lista
# - Expense tracker
# - Simple CRM
# - Inventory system
...
## 3. Čitaj tuđi kod
```python
# GitHub - proučavaj popularne Python projekte
# Stack Overflow - vidi različita rješenja
# Open source - doprinesi projektima
```
...
# Slijedi PEP 8
# Koristi docstrings
# Type hints gdje ima smisla
# Testiraj kod (pytest, unittest)
```

## 5. Učenje kroz učenje
```python
# Objasni drugima što si naučio
# Napiši blog post
# Pomozi na forumima
# Predavanje objašnjava najbolje
```
...
## 1. Mutabilni default argumenti
```python
# ❌ LOŠE
def dodaj_stavku(lista=[]):
    lista.append(1)
    return lista

# ✅ DOBRO
def dodaj_stavku(lista=None):
    if lista is None:
        lista = []
    lista.append(1)
    return lista
...
## 2. Kasno vezanje u petljama
```python
# ❌ LOŠE
funkcije = []
for i in range(5):
    funkcije.append(lambda: i)
# Sve vraćaju 4!

# ✅ DOBRO
funkcije = []
for i in range(5):
    funkcije.append(lambda x=i: x)
```

## 3. Modificiranje liste tijekom iteracije
```python
# ❌ LOŠE
lista = [1, 2, 3, 4, 5]
for x in lista:
    if x % 2 == 0:
        lista.remove(x)  # Može preskočiti elemente!
...
## 4. Nekorištenje super() pravilno
```python
# ❌ LOŠE
class Child(Parent):
    def __init__(self):
        Parent.__init__(self)  # Problemi s multiple inheritance

# ✅ DOBRO
class Child(Parent):
    def __init__(self):
        super().__init__()  # Koristi MRO pravilno
```

## 5. Ignoriranje exceptions
```python
# ❌ LOŠE
try:
    risky_operation()
except:
    pass  # Sakriva sve greške!
...
Ovaj vodič pokriva:
✅ Python osnove (varijable, tipovi, kontrolne strukture)
✅ Funkcije i lambda izrazi
✅ Kolekcije (list, dict, tuple, set)
✅ OOP koncepte (klase, objekti, atributi, metode)
✅ Četiri stupa OOP-a (enkapsulacija, nasljeđivanje, polimorfizam, apstrakcija)
✅ Specijalne metode (magic methods)
✅ Properties i dekoratori
✅ Design patterns (Singleton, Factory, Builder, Observer)
✅ Praktične primjere (knjižnica, e-commerce, banka, ERP)
✅ Best practices i konvencije
✅ Exception handling
✅ Tips & tricks
...
1. **Prođi kroz primjere** - Izvrti svaki kod i eksperimentiraj
2. **Napravi vlastite projekte** - Primijeni naučeno
3. **Proučavaj tuđi kod** - GitHub, open source
4. **Čitaj dokumentaciju** - Python docs su izvrsni
5. **Pridruži se zajednici** - Reddit r/learnpython, Stack Overflow
6. **Vježbaj redovno** - Consistency is key!

## Dodatna područja za učenje:
...
- **Databases**: SQLAlchemy, Django ORM

---

**Sretno učenje! 🐍🚀**

**Autor:** Python i OOP Kompletni Vodič  
**Verzija:** 1.0  
...
1. **Spremi kao**: `python_oop_vodic.md`
2. **Koristi kao referencu** - bookmark često korištene dijelove
3. **Vježbaj primjere** - kopiraj u Python interpreter
4. **Prilagodi svojim potrebama** - dodaj vlastite bilješke
5. **Dijeli s drugima** - učenje kroz podučavanje

**Napomena:** Ovaj vodič je dizajniran kao praktični priručnik. Za dublje razumijevanje pojedinih koncepata, preporučuje se dodatno istraživanje i čitanje službene dokumentacije.

---

**KRAJ DOKUMENTA - SVA 4 DIJELA KOMPLETIRANA! ✅**
...
def progres_bar(poruka="Loading in progres"): 

    for i in range(101):
        time.sleep(0.025)
        print(f"\r{Collors.OKCYAN}{i} % {poruka} {Collors.ENDC}{Collors.BOLD}{Collors.ENDC}", end='', flush=True)
        # time.sleep(0.01)
        if i == 100:
            print(f"\r{Collors.OKCYAN}{i} % {poruka} {Collors.ENDC}{Collors.BOLD}{Collors.ENDC}", end='', flush=True)
            time.sleep(0.2)
...
def progres_bar_unazad(poruka="Čistim memoriju ..."):
    for i in range(100, -1, -1):  # Počinje od 100, ide do -1 (uključuje 0), smanjuje se za 1
        time.sleep(0.05)
        print(f"\r{Collors.OKCYAN}{poruka} {i} % {Collors.ENDC}{Collors.BOLD}{Collors.ENDC}", end='', flush=True)
...
while True:
    ...
    time.sleep(1)
    isprazni_ekran()
    header_blue("Welcome to Rock, Paper, Scissors!")
    info_msg("This is a simple game where you can play against the computer.")
    info_msg("You will be asked to enter your choice (rock, paper, or scissors).")
    info_msg("The computer will randomly choose one of the three options.")
    info_msg("The winner is determined based on the rules of the game.")
    info_msg("You can play multiple rounds until you decide to quit.")
    info_msg("Good luck!")
    ...
    if play_again not in ["yes", "y"]:
       info_msg(f"--------------------------------------")
       info_msg(f"Odabrao si izlaz, doviđenja !")
       info_msg(f"--------------------------------------")
       progres_bar(f" {Collors.BOLD} Izlazim iz programa... {Collors.ENDC}")
       info_msg(f"\n--------------------------------------")
